...
export class MocapScene {
  ...
  private applyMixamoPose(bones: BoneT[]) {
      ...
      for (const bp of order) {
        ...
        const isNearIdentity =
          Math.abs(this.worldQuat.w) > 0.999 &&
          Math.abs(this.worldQuat.x) < 0.03 &&
          Math.abs(this.worldQuat.y) < 0.03 &&
          Math.abs(this.worldQuat.z) < 0.03;

        if (isNearIdentity && bindWorld) {
          this.desiredWorld.copy(bindWorld);
        } else if (bindWorld) {
